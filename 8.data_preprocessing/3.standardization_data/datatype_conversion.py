...
print("# 각 열의 자료형 확인", df.dtypes, sep='\n', end='\n\n')
...
print("'?' 데이터 검색 결과 : ", df['horsepower'][nan_list], sep='\n', end='\n\n')

print("# 누락 데이터('?') np.nan으로 변경 ")
df['horsepower'].replace('?', np.nan, inplace=True)      # '?'을 np.nan으로 변경
print("# horsepower 열의 np.nan 변경 확인", df['horsepower'].unique(), sep='\n', end='\n\n')

nan_list = [int(i) for i, a in df['horsepower'].isna().items() if a is True]
print("'nan' 데이터 검색 결과 : ", df['horsepower'][nan_list], sep='\n', end='\n\n')
...
print("# 범주형을 문자열로 다시 변환 전", df['origin'].dtypes, df['origin'][20:30], sep='\n', end='\n\n')
df['origin'] = df['origin'].astype('category')     
print("# origin 열의 문자열 자료형을 범주형으로 변환 후", df['origin'].dtypes, df['origin'][20:30], sep='\n', end='\n\n')

print("# 범주형을 문자열로 다시 변환")
df['origin'] = df['origin'].astype('str')     
print(df['origin'].dtypes, df['origin'][20:30], sep='\n', end='\n\n')

print("# model year 열의 정수형을 범주형으로 변환")
print('==before==', df['model year'].sample(3), sep='\n', end='\n\n')
df['model year'] = df['model year'].astype('category') 
print('==after==', df['model year'].sample(3), sep='\n', end='\n\n')
